...
def raices_multiples(f, df, d2f, x0, Tol, niter, error_type):
    # Paso 1
    ...
    for k in range(1,int(niter)+1):
        ...
        denom = dfx**2 - fx*d2fx
        if denom==0: raise ZeroDivisionError("Denominador cero.")
    ...
    plt.plot(x,f(x),'ro',markersize=6); plt.title('Raíces Múltiples')
